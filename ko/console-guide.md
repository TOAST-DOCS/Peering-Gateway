...
* 피어 VPC를 경유하여 다른 VPC로 연결을 원하는 경우 피어링에서 제공되는 **라우트** 설정을 이용하여 VM 인스턴스를 경유하여 패킷이 전달 되도록 패킷이 전달 되도록 설정해야 합니다.
...
* 피어 VPC를 경유하여 다른 VPC로 연결을 원하는 경우 피어링에서 제공되는 **라우트** 설정을 이용하여 VM Instance 를 경유하도록 패킷이 전달 되도록 설정해야 합니다.
  > [참고] 라우트 사용 방법은 아래 "공통기능 > 라우트" 내용을 참고하시기 바랍니다. 
...
피어링(피어링, 리전 피어링, 프로젝트 피어링)에서 제공되는 공통된 기능을 설명합니다. 

### 라우트

피어링에서 제공되는 **라우트** 설정을 이용하면 피어 VPC의 VM Instance를 경유하여 다른 VPC로 트래픽을 전달하는 구성을 할 수 있습니다. 피어링의 라우트는 설정은 피어링으로 부터 유입된 모든 트래픽을 처리할 VM Instance의 Port 및 Virtual IP Port를 지정할 수 있습니다. 라우트의 Gateway가 되는 VM Instance에는 Network Virtual Appliance VM을 배치하여 VM Instance내부에서 트래픽 제어를 하고 다른 피어링 으로 트래픽을 전달할 수 있습니다.
* 피어링으로 Hub&Spoke 형태의 VPC 연결을 구성하고 Hub VPC에 위치한 Network Virtual Appliance 로 모든 트래픽을 제어하기 하기 원하는 구성은 피어링의 라우팅 기능을 활용하여 구성할 수 있습니다.

> [참고] 현재는 한국(평촌), 한국(판교) 리전에서만 기능이 제공됩니다.

#### 라우트 생성

1. 라우트 설정을 원하는 피어링 선택
2. 하단 탭 에서 라우트 선택
3. **라우트 생성** 선택
4. 피어링 인 경우 **게이트웨이 구분**을 선택 (프로젝트 피어링, 리전 피어링은 항목이 없습니다.)
   > [참고] **피어 게이트웨이**는 피어링 생성시 선택한 **피어 VPC** 에 라우트를 추가하는 것이며, **로컬 게이트웨이**는 **로컬 VPC** 위치를 의미 합니다.<br>
5. 게이트웨이 선택 후 확인 버튼을 클릭합니다
   > [참고] 게이트웨이는 INSTANCE 와 VIRTUAL_IP 만 선택이 가능합니다.<br>

#### 라우트 삭제

1. 라우트 설정 삭제 원하는 피어링 선택
2. 라우트 선택
3. **라우트 삭제** 버튼을 클릭 합니다.
