...
2. **피어링 생성** 버튼을 클릭합니다.
3. **이름**을 입력하고 **로컬 VPC**, **피어 VPC**를 석택합니다.
...
4. **피어 테넌트**를 선택합니다.
   ...
   > [참고] 피어 테넌트 ID<br>
   > **피어 테넌트 ID** 확인 방법은 하단의 "피어 테넌트 ID 확인" 내용을 참고하시기 바랍니다.
6. **피어 VPC ID**를 입력합니다.</br>
   > [참고] 피어 VPC ID<br>
   > **피어 VPC ID** 확인 방법은 하단의 "피어 VPC ID 확인" 내용을 참고하시기 바랍니다.

#### 피어 VPC ID 확인

리전 피어링을 생성하려면 다른 리전에서 피어링 대상이 되는 VPC의 **VPC ID**를 알아야 합니다. 피어 리전의 VPC ID를 확인하는 방법은 다음과 같습니다.

> [참고] 피어 VPC가 다른 프로젝트인 이며 접근 권한이 없을시 피어 프로젝트의 관리자에게 VPC ID를 제공받기 바랍니다.
...
다른 프로젝트간에 리전 피어링을 생성하려면 다른 리전에서 피어링 대상이 되는 VPC의 **테넌트 ID**를 알아야 합니다. 피어 리전의 테넌트 ID를 확인하는 방법은 다음과 같습니다.
...
> [참고] 프로젝트 피어링 허용 방법은 아래 "피어링 허용 대상 관리" 내용을 참고하시기 바랍니다. 
...
**피어링 허용 대상 관리**를 추가하기 위해서는 피어 프로젝트에서 피어링 대상이 되는 VPC의 **VPC ID**를 알아야 합니다. 다음 절차로 피어 프로젝트의 **VPC ID**를 확인 할 수 있습니다.
...
**피어링 허용 대상 관리**를 추가하기 위해서는 피어 프로젝트에서 **테넌트 ID**를 알아야 합니다. 다음 절차로 피어 프로젝트의 **테넌트 ID**를 확인 할 수 있습니다.
