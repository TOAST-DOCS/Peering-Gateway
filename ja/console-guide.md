...
1. ピアプロジェクトのコンソール画面に接続します。
2. **Network > VPC > 管理**に移動します。
3. ピアリング対象または画面に表示されるVPCのいずれか1つを選択します。
4. **基本情報 > テナントID**に表示されているID値をコピーします。
...
1. ルート設定を行いたいピアリングを選択します。
2. 下部タブでルートを選択します。
3. **ルート変更**を選択します。
   > [参考]ピアリングの場合、2つのボタンが存在します。**ピアルート変更**はピアリング作成時に選択した**ピアVPC**にルートを追加することを意味し、**ローカルルート変更**は**ローカルVPC**の場所を意味します。<br>
4. **+**ボタンをクリックします。
5. 対象CIDRを入力します。
6. ゲートウェイを選択します。
