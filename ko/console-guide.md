...
2. 하단 탭에서 라우트 선택
3. **라우트 생성** 선택
4. 피어링인 경우 **게이트웨이 구분**을 선택(프로젝트 피어링, 리전 피어링은 항목이 없습니다.)
   > [참고] **피어 게이트웨이**는 피어링 생성시 선택한 **피어 VPC**에 라우트를 추가하는 것이며, **로컬 게이트웨이**는 **로컬 VPC** 위치를 의미합니다.<br>
5. 게이트웨이 선택 후 확인 버튼을 클릭합니다.
   > [참고] 게이트웨이는 인스턴스와 가상 IP만 선택이 가능합니다.<br>

#### 라우트 삭제

1. 라우트 설정 삭제를 원하는 피어링 선택
2. 라우트 선택
3. **라우트 삭제** 버튼을 클릭 합니다.
