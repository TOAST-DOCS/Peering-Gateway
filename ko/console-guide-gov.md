...
* 2024.06.04 이전에 생성한 조직 내 프로젝트에서 판교 리전에 생성한 피어링은 **기본 라우팅 테이블**에 연결된 서브넷에서만 통신이 가능합니다.
  > [참고] 피어링을 생성하면 기본 라우팅 테이블에 피어링 통신을 라우팅 하기 위해 상대 VPC의 IP 주소 범위로 향하는 패킷들을 피어링 게이트웨이로 전달하는 라우팅 룰이 묵시적으로 추가됩니다. 따라서 기본 라우팅 테이블에 상대 VPC의 IP 주소 범위를 CIDR로 하는 다른 라우트를 설정할 수 없습니다. 또한, 상대 VPC의 IP 주소 범위 내의 일부를 CIDR로 지정하는 라우트를 추가할 경우 피어링 통신을 위한 라우트보다 우선순위가 높기 때문에 피어링 통신이 되지 않을 수 있으므로 주의가 필요합니다.
* 평촌 리전에서 생성한 피어링과 2024.06.04 이후 생성한 조직 내 프로젝트에서 판교 리전에 생성한 피어링은 피어링한 양쪽 VPC의 라우팅 테이블에 별도의 라우트를 설정해야 통신이 가능합니다.
...
1. **Network > Peering Gateway > 리전 피어링** 또는 **Network > Peering Gateway > 프로젝트 피어링**으로 이동 합니다.
